...
def load_config_from_sheet(service_json, spreadsheet_id):
...
def create_or_replace_tab(spreadsheet, title, rows):
    try:
...
def cleanup_old_tabs(spreadsheet, keep_days=5):
    ...
    for ws in spreadsheet.worksheets():
        ...
        for pattern in patterns:
            ...
            if dt < cutoff:
                ...
                print(f"Deleted old worksheet: {title}")
                break
...
def upload_daily_results(
    ...
    gc = get_client(service_json)
    sh = gc.open_by_key(spreadsheet_id)
    cleanup_old_tabs(sh, keep_days=5)
    today = datetime.utcnow().strftime("%Y_%m_%d")
    create_or_replace_tab(sh, f"ADS_{today}", dicts_to_rows(ads_rows))
    create_or_replace_tab(sh, f"APPADS_{today}", dicts_to_rows(app_rows))
    create_or_replace_tab(sh, f"CTV_{today}", dicts_to_rows(ctv_rows))
